...
public class CharacterCardImportTests
{
    ...
    [Fact]
    public void Parser_accepts_library_bundle_and_keeps_exported_current_card_last()
    {
        var first = DocumentJson("\"id\":\"first\"").Replace("\"name\":\"调查员\"", "\"name\":\"第一张\"");
        var current = DocumentJson("\"id\":\"current\"").Replace("\"name\":\"调查员\"", "\"name\":\"当前卡\"");
        var html = WrapLibrary($$"""
        {
          "schema":"tott-coc7e-investigator-library",
          "version":1,
          "currentId":"current",
          "investigators":[{{current}},{{first}}]
        }
        """);
        ...
        result.SourceSchema.Should().Be("tott-coc7e-investigator-library");
        result.Documents.Select(x => x.Profile.Name).Should().Equal("第一张", "当前卡");
        result.Document!.Profile.Name.Should().Be("第一张");
    }

    [Fact]
    public void Parser_rejects_library_bundle_with_an_invalid_nested_card()
    {
        var invalid = DocumentJson("\"schema\":\"other-schema\",\"version\":1");
        var result = _parser.Parse(WrapLibrary($$"""
        {"schema":"tott-coc7e-investigator-library","version":1,"investigators":[{{invalid}}]}
        """));

        result.Success.Should().BeFalse();
        result.ErrorMessage.Should().Contain("第 1 张").And.Contain("schema");
    }

    [Fact]
    public void Library_bundle_pipeline_maps_and_imports_every_card()
    {
        var first = DocumentJson("\"id\":\"first\"").Replace("\"name\":\"调查员\"", "\"name\":\"第一张\"");
        var current = DocumentJson("\"id\":\"current\"").Replace("\"name\":\"调查员\"", "\"name\":\"当前卡\"");
        var parsed = _parser.Parse(WrapLibrary($$"""
        {
          "schema":"tott-coc7e-investigator-library",
          "version":1,
          "currentId":"current",
        ...
        for (var index = 0; index < parsed.Documents.Count; index++)
        {
            var mapped = _mapper.Map(parsed.Documents[index]);
            mapped.Success.Should().BeTrue();
            processor.ImportCharacterCard(
                246813579,
                mapped.CharacterSheet!,
                CharacterCardConflictPolicy.Rename,
                setAsCurrent: index == parsed.Documents.Count - 1).Success.Should().BeTrue();
        }

        processor.GetCharacterCard(246813579, "第一张").Should().NotBeNull();
        processor.GetCharacterCard(246813579, "当前卡").Should().NotBeNull();
        processor.GetCurrentCharacterCardName(246813579).Should().Be("当前卡");
    }
    ...
    [Fact]
    public void Character_details_include_all_skills_and_export_is_reimportable_st_command()
    {
        var sheet = NewSheet("调查员");
        sheet.Skills["力量"] = 40;

        var details = sheet.CharacterDetails();
        var export = sheet.BuildSkillExportCommand("调查员");

        details.Should().Contain("【技能】")
            .And.Contain("侦查: 50")
            .And.Contain("力量: 40");
        export.Should().StartWith(".st(调查员) ")
            .And.Contain("侦查50")
            .And.Contain("力量40");
    }
    ...
    [Fact]
    public void Onebot_message_attachment_extraction_preserves_batch_bundle_name_for_candidate_detection()
    {
        ...
        [
          {
            "type":"file",
            "data":{
              "file_id":"bundle-file-id",
              "file_name":"CoC7_全部调查员_2026-07-18.html",
              "file_size":12345
            }
          }
        ]
        ...
        var files = OneBotFileInfo.ExtractFromMessageSegments(
            json.RootElement, "group_message", 123456, 654321);

        files.Should().ContainSingle();
        files[0].FileName.Should().Be("CoC7_全部调查员_2026-07-18.html");
        files[0].FileId.Should().Be("bundle-file-id");
        CharacterCardFileImportCoordinator.IsCandidateFile(files[0]).Should().BeTrue();
    }
    ...
    private static string WrapLibrary(string json) =>
        $"<html><body><script id=\"embedded-investigator\" type=\"application/json\">null</script><script type='application/json' id='embedded-investigator-library'>{json}</script><script>throw new Error('must not run')</script></body></html>";
