...
public class AiCharacterSession
{
    ...
    private readonly ThoughtStateMaintainer _thoughtStateMaintainer;
    private readonly EmotionStateMaintainer _emotionStateMaintainer;
        ...
        Action? exitApiScope = null)
    {
        ...
        _thoughtStateMaintainer = new ThoughtStateMaintainer(_context, _apiCaller, _llmCallTracker);
        _emotionStateMaintainer = new EmotionStateMaintainer(_context, _apiCaller, _llmCallTracker);
    ...
    public async Task RespondAsync(long groupId)
    {
        ...
        try
        {
            ...
            await RefreshActionContextAsync(trpgContext, contextInputText);
            ...
            if (recallKeywords != null && recallKeywords.Count > 0)
            {
                ...
                var recentHistory = trpgContext.AgentContextPack?.RecentActiveHistory ?? new List<ChatHistoryEntry>();
                var graphRecall = await _semanticGraphRecall.BuildEvidencePackAsync(
                    Scope,
                    Character.CharacterId,
                    queryText,
                    recentHistory);
                var recallText = graphRecall.ToPromptString();
                if (graphRecall.Hits.Count == 0)
                {
                ...
                var recallResult = await BuildRecallResultStringAsync(Scope, Character.CharacterId, recalls, recallKeywords);
                    recallText = string.IsNullOrWhiteSpace(recallResult)
                        ? string.Empty
                ...
                trpgContext.RecalledMemoryVar = string.Empty;
                RefreshStructuredRecallContext(trpgContext, recallText);
            ...
            if (!string.IsNullOrWhiteSpace(rawRequest))
            {
                ...
                var graphRecall = await _semanticGraphRecall.BuildEvidencePackAsync(
                    ...
                    rawRequest,
                    recentHistory);
                var recallText = graphRecall.Hits.Count > 0
                    ? graphRecall.ToPromptString()
    ...
    private static void RefreshStructuredRecallContext(TrpgPromptContext trpgContext, string recallText)
    {
        if (trpgContext.AgentContextPack == null)
            return;

        trpgContext.AgentContextPack.GraphRecallEvidence = string.IsNullOrWhiteSpace(recallText) ? "无" : recallText;
        trpgContext.StructuredActionContextVar = trpgContext.AgentContextPack.ForActionContextView();
    }


    /// <summary>剥离内部标签(&lt;recall&gt;等)，保留可见文本</summary>
    private async Task RefreshActionContextAsync(TrpgPromptContext trpgContext, string latestText)
    {
        if (trpgContext.AgentContextPack == null)
            return;

        var recentHistory = trpgContext.AgentContextPack.RecentActiveHistory;
        var graphRecall = await _semanticGraphRecall.BuildEvidencePackAsync(
            Scope,
            Character.CharacterId,
            latestText,
            recentHistory);
        var oldState = await _innerStateStore.GetAsync(Scope, Character.CharacterId);
        var thoughtText = await _thoughtStateMaintainer.UpdateAsync(
            Scope,
            Character.CharacterId,
            oldState.ThoughtText,
            graphRecall,
            latestText,
            recentHistory,
            trpgContext.AgentContextPack.CurrentSceneText);
        var emotionText = await _emotionStateMaintainer.UpdateAsync(
            Scope,
            Character.CharacterId,
            oldState.EmotionText,
            thoughtText,
            graphRecall,
            latestText,
            recentHistory);

        await _innerStateStore.SaveAsync(Scope, Character.CharacterId, thoughtText, emotionText);

        trpgContext.AgentContextPack.GraphRecallEvidence = graphRecall.ToPromptString();
        trpgContext.AgentContextPack.ThoughtText = string.IsNullOrWhiteSpace(thoughtText) ? "无" : thoughtText;
        trpgContext.AgentContextPack.EmotionText = string.IsNullOrWhiteSpace(emotionText) ? "无" : emotionText;
        trpgContext.StructuredActionContextVar = trpgContext.AgentContextPack.ForActionContextView();
    }
