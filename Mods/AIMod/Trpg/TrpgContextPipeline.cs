...
public class TrpgContextPipeline
{
    ...
    public async Task<TrpgPromptContext> BuildContextAsync(TrpgScope scope, AiCharacterEntry aiChar, string latestGmText)
    {
        ...
        var activeAffectiveTags = _config.EnableAffectiveTags
            ? await _db.GetActiveAffectiveTagStatesAsync(scope, aiChar.CharacterId, 8)
            : new List<AffectiveTagState>();
        var affectiveStateString = AffectiveTagController.FormatForPrompt(activeAffectiveTags);
        ...
        var playerTableMemory = await _db.SearchPlayerTableMemoryNodesAsync(scope, queryText, limit: 12);
        var characterIcMemories = await _db.GetCharacterMemoriesAsync(scope, aiChar.CharacterId, limit: 12);
        ...
        return new TrpgPromptContext
        {
            ...
            RecalledMemoryVar = recalledMemory,
            ...
            // 四层架构字段
            ObjectivesVar = objectivesString,
            ...
            AffectiveStateVar = affectiveStateString,
            // 叙事记忆层（认知层）
