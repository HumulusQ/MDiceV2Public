...
public class MemoryWatchdog
{
    ...
    private readonly SemanticGraphFoldExtractor _semanticGraphFoldExtractor;
    private readonly SemanticGraphWriter _semanticGraphWriter;
    private readonly GraphPruner _graphPruner;
        ...
        LlmCallTracker? llmCallTracker = null)
    {
        ...
        _semanticGraphWriter = new SemanticGraphWriter(semanticGraphRepository);
        _semanticGraphFoldExtractor = new SemanticGraphFoldExtractor(_context, _apiCaller, _llmCallTracker);
        _graphPruner = new GraphPruner(semanticGraphRepository);
    }
    ...
    public async Task<bool> CheckAndFoldAsync(TrpgScope scope, string characterId)
    {
        ...
        if (activeEntries.Count < thresholdEntries && activeTokens < thresholdTokens)
        {
            _context.Log(LogLevel.Debug,
                $"[AIMod:TRPG] 历史未达到折叠阈值 (Group={groupId}, Char={characterId}, Count={activeEntries.Count}, ActiveTokens={activeTokens}, ThresholdCount={thresholdEntries}, ThresholdTokens={thresholdTokens})");
            return false;
        }
        ...
        try
        {
            var foldResult = await _semanticGraphFoldExtractor.ExtractAsync(scope, characterId, toFold);
            if (foldResult.ParseFailed)
            {
                _context.Log(LogLevel.Warn,
                    $"[AIMod:TRPG] SemanticGraph fold parse failed | Group={groupId} | Char={characterId} | Error={foldResult.Error}");
                return false;
            }

            if (foldResult.MemoryCandidates.Count == 0 && ContainsNarrativeMaterial(toFold))
            {
                _context.Log(LogLevel.Warn,
                    $"[AIMod:TRPG] SemanticGraph fold returned no candidates for a narrative window | Group={groupId} | Char={characterId} | NOT deleting history");
                return false;
            }

            var writeResult = await _semanticGraphWriter.WriteCandidatesAsync(scope, characterId, foldResult.MemoryCandidates);
            ...
            _context.Log(LogLevel.Info,
                $"[AIMod:TRPG] SemanticGraph fold committed | Group={groupId} | Char={characterId} | Candidates={foldResult.MemoryCandidates.Count} | Inserted={writeResult.InsertedMemoryCount} | Reused={writeResult.ReusedMemoryCount} | SurfaceNodes={writeResult.SurfaceNodeCount} | Edges={writeResult.EdgeUpsertCount} | DeletedHistory={idsToDelete.Count}");

            return true;
        }
        catch (Exception ex)
        {
            _context.Log(LogLevel.Error, $"[AIMod:TRPG] SemanticGraph fold failed: {ex.Message}");
            return false;
        }
    }
    ...
    /// </summary>
    private async Task<bool> LegacyDisabledCheckAndFoldAsync(TrpgScope scope, string characterId)
    {
        var groupId = scope.GroupId;
        await _db.DecayMemoryHeatAsync(scope, characterId);
        ...
        try
        {
            ...
            var graphWrittenCount = 0;
            _context.Log(LogLevel.Info,
                $"[AIMod:TRPG] SemanticGraphFoldExtractor wrote {graphWrittenCount} graph memories | Group={groupId} | Char={characterId}");
    ...
    private static bool ContainsNarrativeMaterial(IReadOnlyList<ChatHistoryEntry> entries)
    {
        return entries.Any(entry =>
        {
            ...
            return Regex.IsMatch(content, @"[\p{IsCJKUnifiedIdeographs}A-Za-z0-9]{4,}");
        });
    }
