...
{
    private const string GroupFeatureKey = "customizedReply";

    private static readonly Regex GroupSwitchCommandRegex = new(
        @"^\s*(?:\.customreply|\.cr)(?:\s+(?<action>\S+))?\s*$",
...
#pragma warning restore CS0618
    ...
    public ModMessageResult? OnGroupMessage(long groupId, long userId, string content, bool isAted)
    {
        ...
        try
        {
            // 管理命令必须在群开关关闭后仍可用，否则管理员无法重新开启功能。
            var switchCommandResult = TryHandleGroupSwitchCommand(groupId, userId, content);
            if (switchCommandResult != null)
            {
                return switchCommandResult;
            }

            if (!_context.IsGroupFeatureEnabled(groupId, GroupFeatureKey, defaultValue: true))
            {
                _context.Log(LogLevel.Debug,
                    $"[CustomizedReply] Group switch is off; skipping rules for group {groupId}");
                return null;
            }
            ...
            if (_replyRules.Count == 0)
            {
                _context.Log(LogLevel.Debug, "[CustomizedReply] No reply rules configured; skipping message");
                return null;
            }
    ...
    /// <summary>
    /// 处理群内总开关命令。无参数时只查询状态；改变状态需要群管理或骰子管理权限。
    /// </summary>
    private ModMessageResult? TryHandleGroupSwitchCommand(long groupId, long userId, string content)
    {
        var match = GroupSwitchCommandRegex.Match(content ?? string.Empty);
        if (!match.Success)
        {
            return null;
        }
        ...
        var action = match.Groups["action"].Success
            ? match.Groups["action"].Value.Trim().ToLowerInvariant()
            : "status";
        var currentlyEnabled = _context.IsGroupFeatureEnabled(groupId, GroupFeatureKey, defaultValue: true);

        if (action is "status" or "状态")
        {
            return ModMessageResult.Intercept(
                $"本群自定义回复当前已{(currentlyEnabled ? "开启" : "关闭")}。\n" +
                "群管理员或骰子管理员可使用 .customreply on/off 切换。",
                stopPropagation: true,
                modId: ModId);
        }

        bool? requestedState = action switch
        {
            "on" or "enable" or "开启" or "开" => true,
            "off" or "disable" or "关闭" or "关" => false,
            "switch" or "toggle" or "切换" => !currentlyEnabled,
            _ => null
        };
        ...
        {
            return ModMessageResult.Intercept(
                "用法：.customreply [status|on|off|switch]（短命令：.cr）",
                stopPropagation: true,
                modId: ModId);
        }

        if (!_context.IsGroupAdministrator(groupId, userId) &&
            !_context.IsDiceAdministrator(userId))
        {
            return ModMessageResult.Intercept(
                "权限不足：只有群主、群管理员或骰子管理员可以切换本群的自定义回复。",
                stopPropagation: true,
                modId: ModId);
        }

        if (currentlyEnabled == requestedState.Value)
        {
            return ModMessageResult.Intercept(
                $"本群自定义回复已经是{(currentlyEnabled ? "开启" : "关闭")}状态。",
                stopPropagation: true,
                modId: ModId);
        }

        _context.SetGroupFeatureEnabled(groupId, GroupFeatureKey, requestedState.Value);
        _context.Log(LogLevel.Info,
            $"[CustomizedReply] Group switch changed: Group={groupId}, User={userId}, Enabled={requestedState.Value}");

        return ModMessageResult.Intercept(
            $"已{(requestedState.Value ? "开启" : "关闭")}本群的全部自定义回复。",
            stopPropagation: true,
            modId: ModId);
    }
    ...
    public async Task<ModUpdateResult> CheckAndUpdateFromGitHubAsync(string owner = "HumulusQ", string repo = "MDiceV2Public")
    {
        ...
        try
        {
            ...
            // Core 作为子进程运行时，BaseDirectory 会指向 Core 目录；
            // 使用宿主启动时设定的工作目录，才能与 ModPluginLoader 实际扫描的
            // {应用根目录}/mods 保持一致。
            var modsRoot = Path.Combine(GetModInstallationRootDirectory(), "mods");
    ...
    /// <summary>
    /// 获取 Mod 的实际安装根目录。
    /// Core 启动时会将当前工作目录设置为应用根目录；若该信息不可用，
    /// 则兼容 Core 位于应用根目录下 Core 子目录的发布结构。
    /// </summary>
    private static string GetModInstallationRootDirectory()
    {
        ...
        var trimmedBaseDirectory = appBaseDirectory.TrimEnd(
            Path.DirectorySeparatorChar,
            Path.AltDirectorySeparatorChar);

        return Path.GetFileName(trimmedBaseDirectory).Equals("Core", StringComparison.OrdinalIgnoreCase)
            ? Directory.GetParent(trimmedBaseDirectory)?.FullName ?? trimmedBaseDirectory
