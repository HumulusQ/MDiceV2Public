...
public class ModContextImpl : IModContext
{
    ...
    public bool IsGroupAdministrator(long groupId, long userId)
    {
        try
        {
            ...
            if (processor == null)
            {
                Log(LogLevel.Warn, "MessageProcessor is null, cannot query group administrator state");
                return false;
            }
            return processor.IsGroupAdministrator(groupId, userId);
        }
        catch (Exception ex)
        {
            Log(LogLevel.Error, $"Failed to query group administrator state for group {groupId}, user {userId}: {ex.Message}");
            return false;
        }
    }
    ...
    public bool IsDiceAdministrator(long userId)
    {
        try
        {
            ...
            if (processor == null)
            {
                Log(LogLevel.Warn, "MessageProcessor is null, cannot query dice administrator state");
                return false;
            }
            return processor.IsDiceAdministrator(userId);
        }
        catch (Exception ex)
        {
            Log(LogLevel.Error, $"Failed to query dice administrator state for user {userId}: {ex.Message}");
            return false;
        }
    }
    ...
    public bool IsGroupFeatureEnabled(long groupId, string featureKey, bool defaultValue = true)
    {
        try
        {
            ...
            if (processor == null)
            {
                Log(LogLevel.Warn, "MessageProcessor is null, cannot query group feature state");
                return defaultValue;
            }
            return processor.IsGroupFeatureEnabled(groupId, featureKey, defaultValue);
        }
        catch (Exception ex)
        {
            Log(LogLevel.Error, $"Failed to query group feature '{featureKey}' for group {groupId}: {ex.Message}");
            return defaultValue;
        }
    }
    ...
    {
        var processor = _messageDistribution.MessageProcessor
            ?? throw new InvalidOperationException("MessageProcessor is null, cannot persist group feature state");
        processor.SetGroupFeatureEnabled(groupId, featureKey, enabled);
    }
}
