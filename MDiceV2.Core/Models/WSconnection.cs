...
public sealed record OneBotGroupFileUploadResult(bool Success, string Message);
...
public partial class WSconnection : ObservableObject
{
    ...
    /// 上传群文件并等待 OneBot 确认。
    /// </summary>
    public async Task<OneBotGroupFileUploadResult> UploadGroupFileAsync(
        long groupId,
        string filePath,
        string name = "",
        int timeoutMs = 60000)
    {
        if (groupId <= 0)
        {
            return new(false, "群号无效。");
        }
        if (string.IsNullOrWhiteSpace(filePath))
        {
            return new(false, "本地文件路径为空。");
        }
        ...
        try
        {
            ...
            {
                return new(false, $"本地文件不存在：{fullPath}");
            }
            if (new FileInfo(fullPath).Length <= 0)
            {
                return new(false, $"本地文件为空：{fullPath}");
            }
        }
        catch (Exception ex)
        {
            return new(false, $"无法读取本地文件：{ex.Message}");
        }
        ...
        var response = await SendRequestAndAwaitResponseAsync(request, timeoutMs);
        return ParseUploadGroupFileResponse(response);
    }

    /// <summary>
    /// 解析 OneBot 11 upload_group_file 响应。
    /// </summary>
    public static OneBotGroupFileUploadResult ParseUploadGroupFileResponse(JsonElement? response)
    {
        if (response is null)
        {
            return new(false, "OneBot 上传请求超时、无响应或 WebSocket 已断开。");
        }

        var root = response.Value;
        if (root.ValueKind != JsonValueKind.Object)
        {
            return new(false, "OneBot 返回了无法识别的上传响应。");
        }

        var status = TryGetResponseString(root, "status");
        var wording = TryGetResponseString(root, "wording", "message", "msg");
        var hasRetCode = TryGetResponseInt32(root, "retcode", out var retCode);
        var failed = string.Equals(status, "failed", StringComparison.OrdinalIgnoreCase) ||
                     (hasRetCode && retCode != 0);

        if (failed)
        {
            var code = hasRetCode ? $"（retcode={retCode}）" : string.Empty;
            return new(false, string.IsNullOrWhiteSpace(wording)
                ? $"OneBot 上传群文件失败{code}。"
                : $"OneBot 上传群文件失败{code}：{wording}");
        }

        if (string.Equals(status, "ok", StringComparison.OrdinalIgnoreCase) ||
            (hasRetCode && retCode == 0))
        {
            return new(true, "OneBot 已确认群文件上传成功。");
        }

        return new(false, "OneBot 上传响应缺少 status/retcode，无法确认文件是否上传。");
    }

    private static string? TryGetResponseString(JsonElement root, params string[] propertyNames)
    {
    ...
    private static bool TryGetResponseInt32(JsonElement root, string propertyName, out int value)
    {
        value = 0;
