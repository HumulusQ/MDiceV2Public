...
public partial class MessageProcessor : ObservableObject
{
    ...
    /// #update cr / #update customreply - 更新 CustomizedReply mod
    ...
    private void HandleAuthUpdateCommand(string args, Msg msg)
    {
        ...
        {
            HandleManualGroupUpdateCommand(msg);
        }
        else if (subcommand == "db" || subcommand == "database")
        {
            HandleDatabaseImportCommand(msg);
        }
        else if (subcommand is "cr" or "customreply")
        {
        ...
        {
            Reply("❌ 未知的更新类型。使用: #update (主程序) / #update db / #update cr 或 #update customreply / #update aimod / #update coccard", msg);
        }
    }
    ...
    private async Task TriggerModUpdateAsync(Msg msg)
    {
        try
        {
            ...
            // 更新由主程序直接执行，避免依赖当前已加载 Mod 的版本和安装路径。
...
#pragma warning restore CS4014
    ...
    private async Task<bool> DownloadAndInstallCustomizedReplyModAsync(Action<string> log)
    {
        ...
        try
        {
            ...
            var modsRoot = Path.Combine(applicationRoot, "mods");
    ...
    private void HandleSkillInsert(string args, Msg msg)
    {
        ...
        // .st out [人物卡名] exports every skill in the same command syntax that
        // this program accepts on import.  The card name is optional: when it is
        // omitted, export the user's currently selected card.
        if (tail.Equals("out", StringComparison.OrdinalIgnoreCase) ||
            tail.StartsWith("out ", StringComparison.OrdinalIgnoreCase))
        {
            string requestedName = tail.Length == 3 ? string.Empty : tail[3..].Trim();
            if (requestedName.Length >= 2 && requestedName[0] == '(' && requestedName[^1] == ')')
                requestedName = requestedName[1..^1].Trim();
            string? exportCardName = !string.IsNullOrWhiteSpace(requestedName)
                ? ResolveCardForExport(requestedName, userId)
                : TryGetCurrentCharacterName(userId);

            if (string.IsNullOrEmpty(exportCardName) ||
                !characterSkills.TryGetValue(userId, out var exportCards) ||
                !exportCards.TryGetValue(exportCardName, out var exportSheet))
            {
                Reply(string.IsNullOrWhiteSpace(requestedName)
                    ? "未找到当前人物卡。用法: .st out [人物卡名]"
                    : $"找不到人物卡: {requestedName}", msg);
                return;
            }

            Reply(exportSheet.BuildSkillExportCommand(exportCardName), msg);
            return;
        }
    ...
    private string? ResolveCardForExport(string identifier, long userId)
    {
...
关于详细的人物卡和特殊卡效果，请使用”.rule(duel)[卡牌名称]“查询(如”.rule(duel)哥布林“)，或者直接在对局决策状态下使用”s[卡牌名称]“来快捷查询。";
    ...
    ///      .ww10 a9 +5（投掷后额外加5成功）
    ///      .ww10 a11（不加骰）
    /// 加骰阈值默认为8，可设为8或更高；设为大于10的值时不加骰。
    /// 显式指定的阈值会作为该用户后续 .ww 检定的默认值并持久化。
    /// 
    /// 检定规则：
    ...
    private void HandleWwRoll(string args, Msg msg)
    {
        try
        {
            ...
            int addThreshold = wwAddDiceThresholds.TryGetValue(msg.UserId, out int cachedThreshold)
                ? cachedThreshold
                : defaultAddThreshold;
            bool hasExplicitAddThreshold = false;
            ...
            // 只在整条指令通过基本校验后更新用户设置，避免无效掷骰意外改写配置。
            if (hasExplicitAddThreshold)
            {
                wwAddDiceThresholds[msg.UserId] = addThreshold;
                SaveUserData(msg.UserId);
            }

            // d10 不可能达到大于 10 的阈值。
            bool isAddDiceDisabled = addThreshold > 10;
