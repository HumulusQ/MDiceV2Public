...
public partial class MessageProcessor
{
        ...
        Action<string> log)
    {
        ...
        var exePath = startupMode == StartupMode.Console
            ? Path.Combine(appRoot, "MDiceV2.Console.exe")
            : Path.Combine(appRoot, "MDiceV2.Launcher.exe");
        ...
        bat.AppendLine("if exist \"%EXE_PATH%\" start \"\" \"%EXE_PATH%\"");
        bat.AppendLine("start \"\" cmd /c \"timeout /t 2 >nul ^& del /f /q \"\"%~f0\"\"\"");
