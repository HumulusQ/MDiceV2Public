...
public partial class MessageDistribution : ObservableObject
{
    ...
    /// <summary>
    /// Confirmation for a Master-uploaded group update package.
    /// </summary>
    public Action<Msg, string>? OnQqUpdateConfirmation;
    ...
    private void HandleMessageEvent(JsonElement json)
    {
        ...
        if (messageType == "group")
        {
            ...
            // Some OneBot adapters echo messages sent by the bot back through the
            // normal message event.  Those echoes must never consume a user's
            // focused confirmation (for example, an HTML character-card import).
            if (IsMessageSentByBot(json, userId))
            {
                Log.InfoFormat($"[群消息] 忽略机器人自身回显: 群:{groupId} 用户:{userId}");
                return;
            }

            // OneBot group messages already carry the authoritative sender role.
            // Feed it into the same cache used by permission checks before the
            // message reaches Mods. This also makes the offline debug interface
            // able to validate group-admin commands without a live OneBot API.
            if (json.TryGetProperty("sender", out var sender) &&
                sender.ValueKind == JsonValueKind.Object &&
                sender.TryGetProperty("role", out var roleElement))
            {
                var role = roleElement.GetString();
                if (role is "owner" or "admin" or "member")
                {
                    OnGroupAdmin?.Invoke(groupId, userId, role is "owner" or "admin");
                }
            }
        ...
        else if (messageType == "private")
        {
            ...
            if (IsMessageSentByBot(json, userId))
            {
                Log.InfoFormat($"[好友消息] 忽略机器人自身回显: 用户:{userId}");
                return;
            }
    ...
    /// <summary>
    /// Returns true for a message event emitted by this bot itself.  Prefer the
    /// event's self_id because it is available before the asynchronous login-info
    /// cache has finished initializing; fall back to the cached ID otherwise.
    /// </summary>
    private bool IsMessageSentByBot(JsonElement json, long senderUserId)
    {
        long selfUserId = 0;
        if (json.TryGetProperty("self_id", out var selfIdElement))
        {
            if (selfIdElement.ValueKind == JsonValueKind.Number)
                selfIdElement.TryGetInt64(out selfUserId);
            else if (selfIdElement.ValueKind == JsonValueKind.String)
                long.TryParse(selfIdElement.GetString(), out selfUserId);
        }

        if (selfUserId <= 0)
            selfUserId = _cachedSelfUserId;

        return selfUserId > 0 && senderUserId == selfUserId;
    }
    ...
    public void Reply(string content, Msg msg)
    {
        ...
        // OneBot 不会把机器人自己发出的消息作为入站消息回送给本程序。
        // 因此必须在实际回复出口补写一次群聊机器人消息，否则 .log 只能记录用户输入。
        try
        {
            MessageProcessor?.RecordBotGroupMessage(refined_content, msg);
        }
        catch (Exception ex)
        {
            Log.Warn($"[TRPG日志] 记录机器人回复失败: {ex.Message}");
        }
    ...
    public void ReplyForward(List<(string timestamp, long userId, string senderName, string content)> entries, Msg msg)
    {
        ...
        {
            // OneBot forward messages support the complete review page (50
            // nodes). Build one request so the review remains a single bubble.
            var selfInfo = GetSelfInfo();
            var messages = new List<Dictionary<string, object>>(entries.Count);
            foreach (var entry in entries)
            {
                // System-generated log entries use user_id=0; OneBot rejects
                // invalid node IDs, so map only those entries to the bot while
                // retaining real participant IDs and names unchanged.
                var effectiveUserId = entry.userId > 0 ? entry.userId : selfInfo.UserId;
                var effectiveSenderName = string.IsNullOrWhiteSpace(entry.senderName)
                    ? selfInfo.Nickname
                    : entry.senderName;
    ...
    private void HandleFocusedMessage(string userId, string focusType, Msg msg, string message, bool isSimulationMode = false)
    {
        ...
        if (focusType == "qq_update_confirm")
        {
            OnQqUpdateConfirmation?.Invoke(msg, message);
            return;
        }
