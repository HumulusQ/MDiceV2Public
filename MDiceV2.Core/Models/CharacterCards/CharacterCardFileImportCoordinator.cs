...
public sealed class CharacterCardFileImportCoordinator : IDisposable
{
    ...
    private async Task DownloadAndImportAsync(OneBotFileInfo file, CancellationToken cancellationToken)
    {
        ...
        if (!mapped.Success)
        {
            ...
            SendFailure(file, mapped.ErrorMessage);
            return;
        }

        var requestedName = mapped.CharacterSheet!.Name;
        var imported = _messageProcessor.ImportCharacterCard(
            file.UserId, mapped.CharacterSheet, CharacterCardConflictPolicy.Rename, setAsCurrent: true);
        if (!imported.Success)
        {
            Log.Warn($"[人物卡导入] import failed group={file.GroupId} user={file.UserId} fileId={file.FileId} stage=persist");
            SendFailure(file, imported.Message);
            return;
        }

        Log.Normal($"[人物卡导入] success group={file.GroupId} user={file.UserId} fileId={file.FileId} via={resolved.Source} schema={parsed.Document!.Schema} version={parsed.Document.Version} name={imported.FinalCharacterName} skills={mapped.SkillCount} conflicts={mapped.ConflictCount}");
        SendSuccess(file, imported, mapped, !string.Equals(requestedName, imported.FinalCharacterName, StringComparison.Ordinal));
    }
    ...
    private void SendSuccess(OneBotFileInfo file, CharacterCardImportResult imported, CharacterSheetMappingResult mapped, bool renamed)
    {
        var message = $"✓ 已导入人物卡「{imported.FinalCharacterName}」\n属性：{imported.CharacteristicCount} 项\n技能：{mapped.SkillCount} 项";
        if (renamed) message += "\n原名称已存在，因此自动使用新名称。";
        message += "\n已切换为当前人物卡。";
