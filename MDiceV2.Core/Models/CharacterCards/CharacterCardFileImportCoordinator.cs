...
public sealed class CharacterCardFileImportCoordinator : IDisposable
{
    ...
    private void OnImportConfirmation(Msg msg, string message)
    {
        ...
        // A pending import is valid for exactly one reply: only an explicit "y"
        // approves it. Any other message consumes the confirmation and cancels it,
        // so normal conversation never remains trapped in this focused state.
        ClearPendingImport(pending.File);
