...
namespace MDiceV2.Models
{
    ...
    private async Task<string> GenerateUpdateBatchFile(string version, string tempDicePath)
    {
        ...
        try
        {
            ...
            var parentProcessId = startupMode == StartupMode.Console
                ? MDiceV2.Core.Program.ParentConsoleProcessId?.ToString() ?? string.Empty
                : string.Empty;
            ...
            catch (Exception ex)
            {
                ...
                batBuilder.AppendLine("echo [%date% %time%] [INFO] Batch script entered.>> \"%MDICE_UPDATE_LOG_FILE%\"");
                ...
                batBuilder.AppendLine("set \"PARENT_PID=%MDICE_UPDATE_PARENT_PID%\"");
                ...
                batBuilder.AppendLine("if \"!PARENT_PID!\"==\"\" goto :PARENT_WAIT_DONE");
                batBuilder.AppendLine("call :LOG \"Waiting for parent Console PID !PARENT_PID! to exit before restart.\"");
                batBuilder.AppendLine("set PARENT_WAIT_COUNTER=0");
                batBuilder.AppendLine(":PARENT_WAIT_LOOP");
                batBuilder.AppendLine("tasklist /FI \"PID eq !PARENT_PID!\" | findstr /I \"!PARENT_PID!\" >nul 2>&1");
                batBuilder.AppendLine("if errorlevel 1 goto :PARENT_WAIT_DONE");
                batBuilder.AppendLine("set /a PARENT_WAIT_COUNTER+=1");
                batBuilder.AppendLine("if !PARENT_WAIT_COUNTER! GTR 60 goto :FAIL_PARENT_STILL_RUNNING");
                batBuilder.AppendLine("timeout /t 1 /nobreak >nul");
                batBuilder.AppendLine("goto :PARENT_WAIT_LOOP");
                batBuilder.AppendLine(":PARENT_WAIT_DONE");
                batBuilder.AppendLine("if not \"!PARENT_PID!\"==\"\" call :LOG \"Parent Console has exited; restart is now safe.\"");
                ...
                batBuilder.AppendLine("goto :CLEANUP");
                batBuilder.AppendLine(":FAIL_PARENT_STILL_RUNNING");
                batBuilder.AppendLine("call :FAIL \"Parent Console did not exit within 60 seconds; target was not replaced.\"");
                batBuilder.AppendLine("goto :CLEANUP");
        ...
        private async Task LaunchUpdateProcess(string batPath)
        {
            ...
            try
            {
                ...
                var updaterEnvironment = new Dictionary<string, string>
                {
                    ["MDICE_UPDATE_APP_ROOT"] = pendingScript.AppRoot,
                    ["MDICE_UPDATE_CORE_DIR"] = pendingScript.CoreDirectory,
                    ["MDICE_UPDATE_SOURCE_DICE"] = pendingScript.SourceDice,
                    ["MDICE_UPDATE_TARGET_DICE"] = pendingScript.TargetDice,
                    ["MDICE_UPDATE_TARGET_BACKUP"] = pendingScript.TargetBackup,
                    ["MDICE_UPDATE_RESTART_EXE"] = pendingScript.RestartExecutable,
                    ["MDICE_UPDATE_LOG_DIR"] = pendingScript.LogDirectory,
                    ["MDICE_UPDATE_LOG_FILE"] = pendingScript.LogFile,
                    ["MDICE_UPDATE_VERSION"] = pendingScript.Version,
                    ["MDICE_UPDATE_PARENT_PID"] = pendingScript.ParentProcessId
                };
                ...
                await TryAppendUpdateScriptLogAsync(
                    ...
                    "INFO",
                    $"Launching batch through the Windows shell. Script={batPath}");

                Log($"启动批处理文件: {batPath}");
                Process? process;
                try
                {
                    foreach (var pair in updaterEnvironment)
                    {
                        previousEnvironment[pair.Key] = Environment.GetEnvironmentVariable(pair.Key, EnvironmentVariableTarget.Process);
                ...
                await TryAppendUpdateScriptLogAsync(
                    pendingScript.LogFile,
                    "INFO",
                    $"Batch launcher started successfully. ProcessId={process.Id}");
