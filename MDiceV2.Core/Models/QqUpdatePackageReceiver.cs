...
public sealed class QqUpdateSession
{
    ...
    public long? ExpectedGroupId { get; set; }
...
public sealed class QqUpdatePackageReceiver
{
    ...
    public QqUpdateCommandResult PrepareGroupTrust(long requestedByUserId, long groupId)
    {
        if (groupId <= 0)
        {
            return Fail("This update-upload mode can only be started in a group chat.");
        }

        var versionLabel = $"QQGroupPackage-{DateTime.Now:yyyyMMdd-HHmmss}";
        return PrepareSession(
            QqUpdateMode.MasterGroupTrustMode,
            versionLabel,
            null,
            null,
            requestedByUserId,
            groupId,
            "✅ Group update-package upload is ready.\n" +
            "Please upload MDiceV2.Core.Zip / MDiceV2.Core.Dice / MDiceV2.Core.UpdatePackage to this group within 30 minutes.\n" +
            "Only the Master who started this session and this exact group are accepted.\n" +
            "After validation, reply y in the group to confirm installation; reply n to cancel.");
    }
    ...
    public async Task<QqUpdateFileReceiveResult> OnFileReceivedAsync(OneBotFileInfo fileInfo)
    {
        ...
        if (!fileInfo.IsPrivateMessage && snapshot.Mode != QqUpdateMode.MasterGroupTrustMode)
        {
        ...
        if (snapshot.Mode == QqUpdateMode.MasterGroupTrustMode &&
            (!snapshot.ExpectedGroupId.HasValue || snapshot.ExpectedGroupId.Value != fileInfo.GroupId || fileInfo.IsPrivateMessage))
        {
            _logger($"[QQ update package] rejected group file outside the prepared group: expected={snapshot.ExpectedGroupId}, actual={fileInfo.GroupId}");
            return NoReply();
        }
    ...
    public bool IsReadyForGroupConfirmation(long userId, long groupId)
    {
        ExpireIfNeeded();
        lock (_sync)
        {
            return _session?.Mode == QqUpdateMode.MasterGroupTrustMode &&
                   _session.State == QqUpdateSessionState.ReadyToApply &&
                   _session.RequestedByUserId == userId &&
                   _session.ExpectedGroupId == groupId;
        }
    }

    public QqUpdateCommandResult ConfirmGroupApply(long userId, long groupId)
    {
        return IsReadyForGroupConfirmation(userId, groupId)
            ? Ok("Group update package confirmed.")
            : Fail("No ready group update package belongs to this Master and group.");
    }
    ...
    private static QqUpdateSession? CloneSession(QqUpdateSession? session)
    {
        ...
        return new QqUpdateSession
        {
            ...
            ExpectedGroupId = session.ExpectedGroupId,
