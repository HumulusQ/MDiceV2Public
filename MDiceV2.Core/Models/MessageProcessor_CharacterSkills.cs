...
public partial class MessageProcessor : ObservableObject
{
    ...
    public class CharacterSheet
    {
        ...
        public string COCCharacterDetails()
        {
            ...
            var details = SafeFormatString(format, db, san.ToString(), hp.ToString(), visibleHp, visibleSan);
            return $"{details}\n\n【技能】\n{BuildSkillDetails()}";
        }

        /// <summary>Builds the complete, stable skill list displayed below a card.</summary>
        public string BuildSkillDetails()
        {
            if (Skills.IsEmpty)
                return "（未录入技能）";

            return string.Join("\n", Skills
                .OrderBy(skill => skill.Key, StringComparer.Ordinal)
                .Select(skill => $"{skill.Key}: {skill.Value}"));
        }

        /// <summary>
        /// Produces a command that can be pasted into another MDiceV2 instance to
        /// recreate this card's skill values.
        /// </summary>
        public string BuildSkillExportCommand(string characterName)
        {
            var skills = Skills
                .OrderBy(skill => skill.Key, StringComparer.Ordinal)
                .Select(skill => $"{skill.Key}{skill.Value}");
            return $".st({characterName}) {string.Join(" ", skills)}";
        }
