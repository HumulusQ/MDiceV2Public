...
public partial class MessageProcessor : ObservableObject
{
    ...
    private class GroupDataRecord
    {
        ...
        /// <summary>
        /// 可选功能和 Mod 的群级开关；缺失的键由功能自身决定默认值。
        /// </summary>
    ...
    private void LoadUserData()
    {
        ...
        foreach (var kvp in all)
        {
            ...
            try
            {
                ...
                // 旧用户数据没有此字段时，保持 .ww 的默认 a8 行为。
                if (record.WwAddDiceThreshold is >= 8)
                {
    ...
    /// <summary>
    /// 检查用户是否为群主或群管理员；缓存未命中时通过 OneBot API 查询。
    /// </summary>
    public bool IsGroupAdministrator(long groupId, long userId)
    {
        ...
        return IsGroupAdmin(groupId, userId) || EnsureGroupAdminFromApi(groupId, userId);
    }

    /// <summary>
    /// 骰子管理员包括系统账号、配置的 Master，以及授权等级为 0 或 1 的用户。
    /// </summary>
    public bool IsDiceAdministrator(long userId)
    {
        if (userId <= 0)
        {
            return false;
        ...
        return personAuth.TryGetValue(userId, out var authLevel) && authLevel < 2;
    }

    /// <summary>
    /// 读取保存在 GroupData 记录中的功能开关。
    /// </summary>
    public bool IsGroupFeatureEnabled(long groupId, string featureKey, bool defaultValue = true)
    {
        if (groupId <= 0 || string.IsNullOrWhiteSpace(featureKey))
        {
            return defaultValue;
        }
        ...
        lock (record)
        {
            return record.FeatureSwitches != null &&
                   record.FeatureSwitches.TryGetValue(normalizedKey, out var enabled)
                ? enabled
                : defaultValue;
        }
    }
    ...
    /// <summary>
    /// 更新功能开关并立即保存对应的 GroupData 记录。
    /// </summary>
    public void SetGroupFeatureEnabled(long groupId, string featureKey, bool enabled)
    {
        if (groupId <= 0)
        {
            throw new ArgumentOutOfRangeException(nameof(groupId), "群功能开关需要有效的正数群号。");
        }
        if (string.IsNullOrWhiteSpace(featureKey))
        {
            throw new ArgumentException("功能键不能为空。", nameof(featureKey));
        }

        var record = groupDataRecords.GetOrAdd(groupId,
            id => new GroupDataRecord { GroupId = id, BotEnabled = true });
        var normalizedKey = featureKey.Trim().ToLowerInvariant();

        lock (record)
        {
            record.FeatureSwitches ??= new Dictionary<string, bool>(StringComparer.OrdinalIgnoreCase);
            record.FeatureSwitches[normalizedKey] = enabled;
    ...
    /// <summary>
    /// 记录程序发出的群聊消息。
    /// OneBot 不会将机器人自己的出站消息重新作为入站事件推送回来，
    /// 所以机器人回复必须在出站路径显式写入跑团日志。
    /// </summary>
    internal void RecordBotGroupMessage(string content, Msg sourceMsg)
    {
        if (sourceMsg.Source != MessageSource.group ||
            sourceMsg.GroupId <= 0 ||
            string.IsNullOrWhiteSpace(content) ||
            _trpgLogManager == null ||
            !IsLogEnabled(sourceMsg.GroupId))
        {
            return;
        }

        var selfInfo = MessageDistribution?.GetSelfInfo();
        long botUserId = selfInfo?.UserId > 0 ? selfInfo.UserId : 1001;
        string botName = string.IsNullOrWhiteSpace(selfInfo?.Nickname) ? "机器人" : selfInfo.Nickname;
        string cleanedContent = CleanMessageContent(content);
        if (string.IsNullOrWhiteSpace(cleanedContent))
        {
            return;
        }

        _trpgLogManager.WriteLog(
            sourceMsg.GroupId,
            botUserId,
            botName,
            cleanedContent,
            LogMessageType.Normal);
    }
    ...
    private void HandleBot(string args, Msg msg)
    {
        ...
        bool isPrivateChat = msg.Source == MessageSource.privatechat;

        // 私聊只允许查询当前私聊会话的状态。私聊没有群上下文，因此不能显示群相关
        // 的信任度等信息，也不能通过 .bot on/off/switch 改变任何会话开关。
        if (isPrivateChat)
        {
            if (string.IsNullOrWhiteSpace(trimmedArgs))
            {
                string privateStatus = IsBotEnabled(botStateKey) ? "开启" : "关闭";
                string privateVersion = GetApplicationVersion();
                Reply(SafeFormatString(
                    GlobalFeedbackMessages.FeedbackTemplates["BotPrivateStatus"],
                    privateVersion,
                    privateStatus), msg);
            }
            else
            {
                Reply(GlobalFeedbackMessages.FeedbackTemplates["BotGroupOnly"], msg);
            }

            return;
        }

        bool isToggleCommand = trimmedArgs is "on" or "off";
        bool canToggleBot = msg.IsGroupAdmin ||
                            msg.IsMasterAccount ||
                            (msg.UserAuthLevel.HasValue && msg.UserAuthLevel.Value <= 1);

        // 保留原有的群管理员限制；.bot on/off 额外允许 Master 和 0/1 级白名单用户。
        if ((isToggleCommand && !canToggleBot) || (!isToggleCommand && !msg.IsGroupAdmin))
        {
